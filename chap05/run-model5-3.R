...
d$Income_std <- (d$Income - mean(d$Income)) / sd(d$Income)
d$Y_std <- (d$Y - mean(d$Y)) / sd(d$Y)
data_std <- list(N=N, Sex = d$Sex, Income = d$Income_std, Y = d$Y_std)
model_std <- cmdstan_model(stan_file = 'model/model5-3_std.stan')
fit_std <- model_std$sample(data=data_std,     seed=123, parallel_chains=4)
fit_std$save_object(file='output/result-model5-3_std.RDS')
